...
const { basename, dirname, join, resolve: pathResolve, parse } = require('path');
const { existsSync, lstatSync, accessSync, readdirSync, R_OK, W_OK, appendFileSync } = require('fs');
...
class MTP_KERNEL {
  ...
  async resolvePath({ filePath }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const _filePath = pathResolve(filePath);
  ...
  async getFileInfo({ filePath = null, fileId = null }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        this.mtpNativeModule.GetFileMetaData(
          this.device,
          fileId, (error, fileInfo) => {
            if (error) {
              return resolve({
                data: null,
                error
  ...
  async deleteFile({ filePath = null, fileId = null }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        this.mtpNativeModule.DestroyFile(this.device, _fileId, (error) => {
          if (error) {
            return resolve({
              data: null,
              error
  ...
  async renameFile({ filePath = null, newfileName, fileId = null }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const file = new this.mtpNativeModule.FileT();
        file.id = _fileId;
        file.storageId = this.storageId;

        this.mtpNativeModule.SetFileName(
          this.device,
          file,
          newfileName, async (error, renamed) => {
            if (error) {
              return resolve({
                data: null,
                error
    ...
    return new Promise(async (resolve) => {
      try {
        if (!undefinedOrNull(newFolderPath)) {
          const _newFolderPath = pathResolve(newFolderPath);
          ...
          this.mtpNativeModule.CreateFolder(
            this.device,
            _newFolderName,
            resolvePathData.id,
            this.storageId, async (error, createdFolder) => {
              if (error) {
                return resolve({
                  data: null,
                  error
              ...
              return resolve(this.processCreatedFolder(createdFolder, _newFolderName, resolvePathData.id));
            }
          );
        } else {
          this.mtpNativeModule.CreateFolder(
            ...
            this.storageId, async (error, createdFolder) => {
              ...
              return resolve(this.processCreatedFolder(createdFolder, _newFolderName, parentId));
            }
          );
        }
      } catch (e) {
  ...
  async processCreatedFolder(createdFolder, newFolderName, parentId) {
    return new Promise(async (resolve) => {
      if (createdFolder === 0) {
        const {
  ...
  async listMtpFileTree({
    ...
    return new Promise(async (resolve) => {
      const filePath = pathResolve(folderPath);
    ...
    return new Promise(async resolve => {
      try {
        ...
        setInterval(() => {
          ...
          appendFileSync('/Users/ganeshr/Desktop/test.txt', `${count++}\n`, 'utf-8', function(err) {

          });
        }, 10);

        const downloadedFile = this.mtpNativeModule.GetFileToFile(
          this.device,
          file.id,
          destinationFilePath,
          (sent, total) => {
  ...
  async uploadFileTree({
    ...
    try {
      ...
      if (!undefinedOrNull(destinationFilePath)) {
        const filePath = pathResolve(dirname(destinationFilePath));
